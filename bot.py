...
TOKEN = os.environ.get('BOT_TOKEN', '8804586061:AAEgbLnXAFH34y1TV9Y325nGcf4iffoIySs')
...
def handle_callback_queries(call):
    ...
    if data == "ui_archive":
        ...
        session['hijri_week'] = data.replace("arch_w_", "")
        user_sessions[user_id] = session
    ...
    # ==========================================
        ...
        session['hijri_month'] = str((datetime.now().month % 12) + 1)
        ...
        markup.add(*buttons)
        markup.add(types.InlineKeyboardButton("⬅️ العودة للقائمة الرئيسية", callback_data="go_home"))
        bot.send_message(user_id, "📅 اختر الشهر الهجري للتصنيف اليدوي:", reply_markup=markup)
...
def fetch_and_send_archive_by_time(user_id, type_search, value_search):
    ...
    for r in rows:
        ...
        try:
            ...
            bot.send_message(user_id, f"⚠️ خطأ في إرسال ملف: {str(e)}")
